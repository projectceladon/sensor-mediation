...
/**
 * id_map, holds list of sensor information to map during iio initalization.
...
/**
 * iioClient:init, establish nw bakend connection with iiod server
...
#if 0
        //sleep(1);
        const char* device_name = devlist[id].name;;
        ALOGD("poll: %s -> data[%d](%f, %f, %f) scale(%f) sampling_period_us(%lld)",
               device_name, devlist[id].raw_channel_count, data[event_count].data[0],
               data[event_count].data[1], data[event_count].data[2],
               devlist[id].scale, (long long) devlist[id].sampling_period_us);
#endif
